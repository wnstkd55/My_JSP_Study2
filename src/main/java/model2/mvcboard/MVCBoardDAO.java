...
public class MVCBoardDAO extends DBConnPool {
    ...
    //주어진 일련 번호에 해당하는 게시물의 조회수를 1 증가시킴
	...
	public int updatePost(MVCBoardDTO dto) {
		...
		return result;		//result > 0 : 수정 성공, result = 0 : 수정 업데이트 실패
	}
	//데이터 삭제 (delete)
	...
	//비밀번호 확인하는 메소드(입력한 비밀번호가 DataBase에 있는 비밀번호와 일치하는지 확인)
	public boolean confirmPassword(String pass, String idx) {
		boolean isCorr = true;
		
		try {
				//COUNT(*) : 레코드가 존재하면 client에서 넘긴 pass, idx DB에 존재
				//COUNT(*) : 레코드가 존재하지 않으면 client에서 넘긴 값이 DB에 존재하지 않는다.
